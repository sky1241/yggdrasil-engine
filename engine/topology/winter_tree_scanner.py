...
def scan_chunk(tree, cid_to_idx):
    ...
    # -- Accumulateurs --
    # cooc[time_key][(a, b)] = weighted_sum   (a < b toujours)
        ...
        try:
            with gzip.open(gz_path, "rt", encoding="utf-8", errors="replace") as f:
                for line in f:
                    ...
                    # --- FILTRE: skip erratum, retraction, retracted ---
                    ...
                    # Co-occurrences (toutes les paires, poids = 1/C(n,2))
                    # Chaque paper distribue exactement 1 point au total
                    if len(indices) >= 2:
                        ...
                        for i in range(n):
                            for j in range(i + 1, n):
                                cooc[tk][(unique[i], unique[j])] += weight
                                pairs_total += 1
    ...
    # Co-occurrences -> JSON gzippe (poids arrondis a 6 decimales)
    cooc_out = {}
    for tk, pairs in cooc.items():
        cooc_out[tk] = {f"{a}|{b}": round(w, 6) for (a, b), w in pairs.items()}
